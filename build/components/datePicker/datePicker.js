...
export class DatePicker extends HTMLElement {
    ...
    constructor(input, _option = null) {
        super();
        ...
        this.section.querySelector('.weekdays').innerHTML = this.option.dayOfWeek.map(x => `<div>${x}</div>`).join('') + `</div>`;
        ...
        this.section.querySelector('.year').onchange = e => {
    ...
    createDom(date) {
        ...
        this.section.querySelector('.year').value = this.currentDate.getFullYear().toString();
        this.section.querySelector('.month').textContent = this.option.months[this.currentDate.getMonth()];
        ...
        while (targetDate <= theLastDay) {
            for (let i = 0; i < 7; i++) {
                ...
                html += `<div class="day ${isOutOfMonth ? 'outOfMonth' : ''}  ${targetDate.getTime() == this.selectedDate.getTime() ? 'selected' : ''}" data-time="${targetDate.getTime()}">${targetDate.getDate()}</div>`;
